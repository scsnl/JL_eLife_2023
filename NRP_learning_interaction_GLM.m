...
% and untrained problems in ASD project
%
% Jin Liu
% 1/6/2021
%
    ...
    for k=1:length(behavior)
        ...
        for l=1:length(image_name)
            if ismember(image_name(l),Image_ASD_sublist)
                temp_change{l,1} = fullfile(oak_path,'results','taskfmri','groupstats','rsa',[contrast{u} '_pre_VS_' contrast{u} '_post'],'analysis',['NRD_' group{1}],[num2str(image_name(l)),'_NRD_plasticity.nii']);
