...
% ACC - interaction
ROIDef_name{1,1}={'R pPHG';'L pPHG';'R FEF';'R IPS';'R MOG';'R MFG'};
ROIDef{1} = [30 -48 2 5];
ROIDef{2} = [-16 -36 -14 5];
ROIDef{3} = [28 -20 60 5];
ROIDef{4} = [30 -64 50 5];
        ...
        if ~isempty(ROIDef)  
            ...
            if exist(input)
                ...
                for h=1:num_ROI
                    ...
                    beh_corr_scatter_2group(x1,y1,x2,y2,'NRP',['ASD: r = ' num2str(r1,3) ' p = ' num2str(p1,3)],['TD: r = ' num2str(r2,3) ' p = ' num2str(p2,3)],Behavior_new_name_pic{i},[ROIDef{h} ' ' type{u}],output_name)
                end
            end
